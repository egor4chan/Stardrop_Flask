...
function create_empty() {
    ...
    div.setAttribute('id', 'no')
    div.innerHTML = 'Здесь будет история Ваших транзакций'
    list_div.appendChild(div)
}

function create_transaction_html(type_value, status_value, amount_value) {
    let list_div = document.getElementById('list')
    let trans = document.createElement('div')
    list_div.appendChild(trans)

    let type = document.createElement('span')
    type.setAttribute('id', 'trans-type')

    let status = document.createElement('span')
    status.setAttribute('id', 'trans-status')

    let amount = document.createElement('span')
    amount.setAttribute('id', 'trans-amount')

    let amount_number = document.createElement('a')
    amount_number.setAttribute('id', 'trans-amount-value')

    trans.appendChild(type)
    trans.appendChild(status)
    trans.appendChild(amount)
    amount.appendChild(amount_number)
...
function get_transactions_user() {
    ...
    httpRequest.onprogress = function() {
        ...
        for (var index = 0; index < list_len; index++) {
            ...
            let a = list[index]['amount']
            let s = list[index]['status']
            let t = list[index]['type']

            create_transaction_html(t, s, a)
        }
    }
